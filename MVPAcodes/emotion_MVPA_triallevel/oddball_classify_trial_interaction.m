...
for t = 1:371
    ...
    g1data = squeeze(double(mean(subjs_oddball_trial_ERP(:,type1,zoom,t:t+4,:),4)));  % set time window
    g2data = squeeze(double(mean(subjs_oddball_trial_ERP(:,type2,zoom,t:t+4,:),4)));
...
%     label = [ones(size(reshaped_g1data,1),1);-1*ones(size(reshaped_g2data,1),1)];
    ...
    for i = 1:100 % 每个被试的4/5trial为训练集，1/5trial为测试集
        ...
        for fold = 1:5
            tem_new_data = squeeze(data_all(i,:,:));
    ...
    for fold = 1:5
        % extract data
        test_data = squeeze(test_datas(fold,:,:));
        train_data = squeeze(train_datas(fold,:,:));
        test_label = squeeze(test_labels(fold,:))';
        train_label = squeeze(train_labels(fold,:))';
        % delete nan data
