...
pw=x.windowRect(3);  %screen size pixel
ph=x.windowRect(4);
...
if block == 1
    ...
    practice_end = imread('stimuli_oddball/practice_end.png');
...
% count soldier_num and identity
        ...
        % show pictrue
        if id==0 
            StimPic = imread(sprintf('stimuli_oddball/exp/target_distractors/other_%d.png',order_line));
            ShowPic(StimPic);
        else
        ...
        % show number of soldier and people
            Screen('TextSize',x.window,fontsize);
    switch order_line
        case 1
            UP_num = soldier_num;
            DOWN_num = probe_num;
        case 2
            UP_num = probe_num;
            DOWN_num = soldier_num;
    end
%             DrawFormattedText(x.window, double(num2str(UP_num)), x.x_center+27, 522, [255, 255, 255]);
%             DrawFormattedText(x.window, double(num2str(DOWN_num)), x.x_center+27, 612, [255, 255, 255]);
%         
