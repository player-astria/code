...
%addpath(genpath('C:\Matlab_Addon\EEElab_your_version\'))
...
%"THE GREAT CHANGE"
read_dir = 'C:\DataFolder\';   %There is your EEG raw data. Please note that you have to adjust the read in function for different file formats. Here the gui of EEGlab might help to identify the function that you need. Remember that everything you clicked in the GUI can be seen as code if you use the code "eegh"

%"THE GREAT CHANGE"
write_dir = 'C:\\Targetfolder\\';           %This is the directory 
...
%"THE GREAT CHANGE"
files = dir([read_dir '*.vhdr']); % This is the file format of brain vision (.vhdr [vmrk,eeg]), if you have another file format, insert the corresponding file type
...
%Number_of_EEG_electrodes_without_reference_and_ground = 32;
...
for VP = 1:COUNTPARTICIPANT  %FOR EVERY FILE
	...
	for checkfilesi = 1:size(checkfiles)	%for every file in there
		...
		if strcmp(strrep(filenames{VP},'.vhdr',''),strrep(checkfilenames{checkfilesi},'.set','')) == 1 	%check whether the present file is in there
    ...
    EEG = pop_loadbv(read_dir, filenames{VP});	%LOAD THE DATA, all channels, all timepoints		
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 0,'setname',filenames{VP},'gui','off'); %in dataset 0 of EEGLab
    ...
    EEG=pop_chanedit(EEG, 'lookup','G:\\Matlab_Addon\\eeglab14_1_1b\\plugins\\dipfit2.3\\standard_BESA\\standard-10-5-cap385.elp','append',Number_of_EEG_electrodes_without_reference_and_ground,'changefield',{Number_of_EEG_electrodes_without_reference_and_ground+1 'labels' 'Cz'},'lookup','G:\\Matlab_Addon\\eeglab14_1_1b\\plugins\\dipfit2.3\\standard_BESA\\standard-10-5-cap385.elp');
    ...
    seperate_script_for_first_segmentation % this file needs to be put into the EEGlab Folder
