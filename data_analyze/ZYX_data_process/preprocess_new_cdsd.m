...
raw_dir = 'D:\ISRR-EEG_data_analysis\Formal EEG data analysis\Data\EEG raw data';
preprocessed_dir = 'D:\ISRR-EEG_data_analysis\Formal EEG data analysis\Data\EEG preprocessed data';
% cd(raw_dir);
% raw_dir_list = dir(raw_dir);
% raw_dir_list = raw_dir_list(3:34);
% raw_dir_list_name = {raw_dir_list.name};
% decision_cdsd_cd_list = {};
% decision_cdsd_sd_list = {};
% decision_sdcd_cd_list = {};
% decision_sdcd_sd_list = {};
% rating_1_list = {};
% rating_2_list = {};
% decision_cdsd_cd_dir = 'D:\ISRR-EEG_data_analysis\Formal EEG data analysis\Data\EEG raw data\decision\cdsd\cd';
% decision_cdsd_sd_dir = 'D:\ISRR-EEG_data_analysis\Formal EEG data analysis\Data\EEG raw data\decision\cdsd\sd';
% decision_sdcd_cd_dir = 'D:\ISRR-EEG_data_analysis\Formal EEG data analysis\Data\EEG raw data\decision\sdcd\cd';
% decision_sdcd_sd_dir = 'D:\ISRR-EEG_data_analysis\Formal EEG data analysis\Data\EEG raw data\decision\sdcd\sd';
% rating_1_dir = 'D:\ISRR-EEG_data_analysis\Formal EEG data analysis\Data\EEG raw data\rating\condition1';
% rating_2_dir = 'D:\ISRR-EEG_data_analysis\Formal EEG data analysis\Data\EEG raw data\rating\condition2';
% for i = 1:size(raw_dir_list,1)
%     raw_subj_folder = raw_dir_list_name{i};
%     cd(raw_subj_folder);
%     current_folder = pwd;
%     cdt_files = dir(fullfile(current_folder,'*.cdt'));
%     ceo_files = dir(fullfile(current_folder,'*.ceo'));
%     dpo_files = dir(fullfile(current_folder,'*.dpo'));
%     cdt_files_name = {cdt_files.name};
%     ceo_files_name = {ceo_files.name};
%     dpo_files_name = {dpo_files.name};
%     decision_cdsd_cd = {};
%     decision_cdsd_sd = {};
%     decision_sdcd_cd = {};
%     decision_sdcd_sd = {};
%     rating_1 = {};
%     rating_2 = {};
%     for j = 1:length(cdt_files_name)
%         if contains(cdt_files_name{j},'cdsd')
%             if contains(cdt_files_name{j},'_cd_')
%                 decision_cdsd_cd =[decision_cdsd_cd; cdt_files_name{j}];
%                 copyfile(cdt_files_name{j},decision_cdsd_cd_dir);
%                 copyfile(ceo_files_name{j},decision_cdsd_cd_dir);
%                 copyfile(dpo_files_name{j},decision_cdsd_cd_dir);
%             elseif contains(cdt_files_name{j},'_sd_')
%                 decision_cdsd_sd =[decision_cdsd_sd; cdt_files_name{j}];
%                 copyfile(cdt_files_name{j},decision_cdsd_sd_dir);
%                 copyfile(ceo_files_name{j},decision_cdsd_sd_dir);
%                 copyfile(dpo_files_name{j},decision_cdsd_sd_dir);
%             end
%         elseif contains(cdt_files_name{j},'sdcd')
%             if contains(cdt_files_name{j},'_cd_')
%                 decision_sdcd_cd =[decision_sdcd_cd; cdt_files_name{j}];
%                 copyfile(cdt_files_name{j},decision_sdcd_cd_dir);
%                 copyfile(ceo_files_name{j},decision_sdcd_cd_dir);
%                 copyfile(dpo_files_name{j},decision_sdcd_cd_dir);
%             elseif contains(cdt_files_name{j},'_sd_')
%                 decision_sdcd_sd =[decision_sdcd_sd; cdt_files_name{j}];
%                 copyfile(cdt_files_name{j},decision_sdcd_sd_dir);
%                 copyfile(ceo_files_name{j},decision_sdcd_sd_dir);
%                 copyfile(dpo_files_name{j},decision_sdcd_sd_dir);
%             end
%         elseif contains(cdt_files_name{j},'_rating_')
%             if contains(cdt_files_name{j},'condition1_')
%                 rating_1 =[rating_1; cdt_files_name{j}];
%                 copyfile(cdt_files_name{j},rating_1_dir);
%                 copyfile(ceo_files_name{j},rating_1_dir);
%                 copyfile(dpo_files_name{j},rating_1_dir);
%             elseif contains(cdt_files_name{j},'condition2_')
%                 rating_2 =[rating_2; cdt_files_name{j}];
%                 copyfile(cdt_files_name{j},rating_2_dir);
%                 copyfile(ceo_files_name{j},rating_2_dir);
%                 copyfile(dpo_files_name{j},rating_2_dir);
%             end
%         end
%     end
%     decision_cdsd_cd_list = [decision_cdsd_cd_list; decision_cdsd_cd];
%     decision_cdsd_sd_list = [decision_cdsd_sd_list; decision_cdsd_sd];
%     decision_sdcd_cd_list = [decision_sdcd_cd_list;  decision_sdcd_cd];
%     decision_sdcd_sd_list = [decision_sdcd_sd_list; decision_sdcd_sd];
%     rating_1_list = [rating_1_list; rating_1];
%     rating_2_list = [rating_2_list; rating_2];
%     cd(raw_dir);
% end

% Dictionary settings
% 选择处理的数据
sellect_data = '\decision\cdsd\sd\';
read_dir = strcat(raw_dir,sellect_data);   % 读取EEG raw data
write_dir = strcat(preprocessed_dir,sellect_data);    % 存储预处理后的EEG data
...
for VP = 1:COUNTPARTICIPANT  %FOR EVERY FILE
    ...
    savename = strcat(write_dir,'removed_channels_auto.mat');  %save the bad channel array for every participant in a matrix 把每个被试的坏导存储为.mat文件
...
%     block_num = filenames{VP}(strfind(filenames{VP},'.cdt')-2:strfind(filenames{VP},'.cdt')-1);
%     if strcmp(block_num, '01') || strcmp(block_num, '02') || strcmp(block_num, '03')
%         decision_cd_segmentation;
%     elseif strcmp(block_num, '04') || strcmp(block_num, '05') || strcmp(block_num, '06')
%         decision_sd_segmentation;
%     end
    
    %都是cd，直接用decision_cd_segmentation
    decision_sd_segmentation;
...
mkdir('data quality');
cd('data quality');
save([sellect_data(11:14) '_' sellect_data(16:17)], '-v7.3')